/*SELECT * from sys.dm_pdw_exec_requests WHERE status = 'Running' AND resource_allocation_percentage IS NOT NULL
...
req.command,
[text].query_sql_text AS 'matched_query_store_text'
...
GROUP BY req.command,req.request_id,req.total_elapsed_time,req.session_id,[text].query_sql_text
order by req.total_elapsed_time desc
OPTION(LABEL='PowerBI')
*/
--Version aggregating all similar SQL_Text
--Calculate approximate time remaining for running queries based on hits on teh query text in query store. 
SELECT
req.session_id,
req.request_id,
